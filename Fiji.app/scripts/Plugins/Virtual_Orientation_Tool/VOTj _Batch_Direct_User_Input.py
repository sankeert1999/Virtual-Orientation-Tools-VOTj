...
Win.addChoice("Alignment with object pointing to", ["Any", "Left (for horizontal) / Top (for vertical)", "Right (for horizontal) / Bottom (for vertical)"], prefs.get("Alignement with object pointing to", "Any"))
...
if Win.wasOKed():  
    ...
    if len(mask_imgs_lst) == len(input_imgs_lst):		
        for file_serial in range(len(mask_imgs_lst)):
            ...
            for mask_file_name in mask_imgs_lst:
                if input_file_name.lower().startswith(mask_file_name.lower()[:mask_file_name.lower().rfind("_mask")]):
        ...
        error = "The number of image files in the mask and image folders is unequal. Please check the respective folders."
